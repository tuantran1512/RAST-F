      ...
      ! ----------------------------------------------------------------------------------------------
      SUBROUTINE GEN_H2O_TABLE(Mpa_in)
      use Inc_H2O, only: core_mpa, cp, del_h, del_ho, del_t, den, h, h_den,      &
                         h_grid, h_t, if_h2o_table_done, k, max_h, max_t, min_h, &
                         min_t, mu, n_h, n_t, t_grid
      implicit none
      REAL(8),INTENT(IN) :: mpa_in
      INTEGER(4)         :: ii
      INTEGER(4)         :: jj
      REAL(8)            :: mpa
      REAL(8)            :: frac
      ...
      LOGICAL(1)         :: if_Exist
         ...
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [GEN_H2O_TABLE] in Mod_H2O'
      ...
      if (abs(MPA_in-CORE_MPA)>1.0d-5) then
         if_h2o_Table_done=.false.
      endif
      if (if_h2o_Table_done) then
         goto 999
      endif
      ...
      do ii=1,n_T
         T_grid(ii)=min_T+del_T*(ii-1)
      enddo ! it
      T_grid(n_T+1)=T_grid(n_T)

      do ii=1,n_T
         den(ii)=water_pt2d(mpa,t_grid(ii))             ! g/cc
         h  (ii)=water_pt2h(mpa,t_grid(ii))*1.0D+3    ! J/kg
         k  (ii)=water_pdt2c(mpa,den(ii),t_grid(ii))    ! W/m-K
         mu (ii)=water_PDT2Eta(mpa,den(ii),t_grid(ii))  ! Pa-s
         cp (ii)=water_pt2cv(mpa,t_grid(ii))*1.0D+3   ! J/kg-K
      enddo ! ir
      den(n_T+1)=den(n_T)
      h  (n_T+1)=h  (n_T)
      k  (n_T+1)=k  (n_T)
      ...
      ! Function of h
      ! -------------------------------------------------------------------------
      if(allocated(h_grid)) deallocate(h_grid)
      if(allocated(h_t   )) deallocate(h_t   )
      if(allocated(h_den )) deallocate(h_den )
      allocate(h_grid(n_h+1))
      allocate(h_t   (n_h+1))
      allocate(h_den (n_h+1))
      h_grid=0.0d0
      h_t   =0.0d0
      h_den =0.0d0
      min_h=h(1)
      max_h=h(n_T)
      del_h=(max_h-min_h)/real((n_h-1),8)
      del_ho=1.0d0/del_h

      do ii=1,n_h
         h_grid(ii)=min_h+del_h*(ii-1)
      enddo ! it
      h_grid(n_h+1)=h_grid(n_h)
      ...
      do ii=1,n_h
         ...
         tmp_T=0.0d0
         do jj=1,n_T-1
            if(h(jj)-eps<=tmp_h .and. tmp_h<=h(jj+1)+eps) then
               frac=(h(jj+1)-tmp_h)/(h(jj+1)-h(jj))
               tmp_T=frac*T_grid(jj)+(1.0d0-frac)*T_grid(jj+1)
               if_exist=.true.
            endif
         enddo ! jj
         if(if_exist) then
            h_t(ii)  =tmp_T
            !h_den(ii)=water_pt2d(mpa,tmp_T) ! g/cc
            h_den(ii)=h2o_t2d(tmp_T) ! g/cc
         else
            write(*,*) 'Cannot find temperature for h=',tmp_h
         endif
      enddo ! ir
      ! -------------------------------------------------------------------------

      if_h2o_Table_done=.true.

999   continue
      return
      END SUBROUTINE GEN_H2O_TABLE
      ! ----------------------------------------------------------------------------------------------

      ! ----------------------------------------------------------------------------------------------
      FUNCTION H2O_T2D(t_in)
      ! Input:  Temperature (K)
      ! output: Density (g/cc)
      use Inc_H2O, only: del_t, del_to, den, max_t, min_t
      implicit none
      REAL(8),INTENT(IN)                          :: T_in
      REAL(8)                                     :: T
      INTEGER(4)                                  :: ii
      REAL(8)                                     :: frac
      REAL(8)                                     :: H2O_T2D                              ! output

#ifdef siarhei_plot
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [H2O_T2D] in Mod_H2O'
...
#ifdef siarhei_fr
      H2O_T2D = 1.d0 !@$^ siarhei_fr_test
#else
      t=max(min_T,min(max_T,t_in))
      ii=int((T-min_T)*del_to+1)
      frac=(min_T+del_T*(ii)-T)*del_to
      H2O_T2D=den(ii)*frac+den(ii+1)*(1.0d0-frac)
#endif
      return
      END FUNCTION H2O_T2D

      FUNCTION H2O_T2H(t_in)
      ! Input:  Temperature (K)
      ! output: Enthalpy (J/kg)
      use Inc_H2O, only: del_t, del_to, h, max_t, min_t
      implicit none
      REAL(8),INTENT(IN)                          :: T_in
      REAL(8)                                     :: T
      INTEGER(4)                                  :: ii
      REAL(8)                                     :: frac
      REAL(8)                                     :: H2O_T2H                              ! output

#ifdef siarhei_plot
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [H2O_T2H] in Mod_H2O'
...
#ifdef siarhei_fr
      H2O_T2H = 2.d5 !@$^ siarhei_fr_test
#else
      t=max(min_T,min(max_T,t_in))
      ii=int((T-min_T)*del_to+1)
      frac=(min_T+del_T*(ii)-T)*del_to
      H2O_T2H=h(ii)*frac+h(ii+1)*(1.0d0-frac)
#endif
      return
      END FUNCTION H2O_T2H

      FUNCTION H2O_T2K(t_in)
      ! Input:  Temperature (K)
      ! output: Thermal conductivity(W/m-K)
      use Inc_H2O, only: del_t, del_to, k, max_t, min_t
      implicit none
      REAL(8),INTENT(IN)                          :: T_in
      REAL(8)                                     :: T
      INTEGER(4)                                  :: ii
      REAL(8)                                     :: frac
      REAL(8)                                     :: H2O_T2K                              ! output

#ifdef siarhei_plot
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [H2O_T2K] in Mod_H2O'
...
#ifdef siarhei_fr
      H2O_T2K = 10.d0 !@$^ siarhei_fr_test
#else
      t=max(min_T,min(max_T,t_in))
      ii=int((T-min_T)*del_to+1)
      frac=(min_T+del_T*(ii)-T)*del_to
      H2O_T2K=k(ii)*frac+k(ii+1)*(1.0d0-frac)
#endif
      return
      END FUNCTION H2O_T2K

      FUNCTION H2O_T2MU(t_in)
      ! Input:  Temperature (K)
      ! output: Viscosity (Pa-s)
      use Inc_H2O, only: del_t, del_to, max_t, min_t, mu
      implicit none
      REAL(8),INTENT(IN)                          :: T_in
      REAL(8)                                     :: T
      INTEGER(4)                                  :: ii
      REAL(8)                                     :: frac
      REAL(8)                                     :: H2O_T2MU                             ! output

#ifdef siarhei_plot
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [H2O_T2MU] in Mod_H2O'
...
#ifdef siarhei_fr
      H2O_T2MU = 1.d-5 !@$^ siarhei_fr_test
#else
      t=max(min_T,min(max_T,t_in))
      ii=int((T-min_T)*del_to+1)
      frac=(min_T+del_T*(ii)-T)*del_to
      H2O_T2MU=mu(ii)*frac+mu(ii+1)*(1.0d0-frac)
#endif
      return
      END FUNCTION H2O_T2MU

      FUNCTION H2O_T2Cp(t_in)
      ! Input:  Temperature (K)
      ! output: Capacity (J/kg-K)
      use Inc_H2O, only: cp, del_t, del_to, max_t, min_t
      implicit none
      REAL(8),INTENT(IN)                          :: T_in
      REAL(8)                                     :: T
      INTEGER(4)                                  :: ii
      REAL(8)                                     :: frac
      REAL(8)                                     :: H2O_T2Cp                             ! output

#ifdef siarhei_plot
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [H2O_T2Cp] in Mod_H2O'
...
#ifdef siarhei_fr_test
      H2O_T2Cp = 4.d3 !@$^ siarhei_fr_test
#else
      t=max(min_T,min(max_T,t_in))
      ii=int((T-min_T)*del_to+1)
      frac=(min_T+del_T*(ii)-T)*del_to
      H2O_T2Cp=Cp(ii)*frac+Cp(ii+1)*(1.0d0-frac)
#endif
      return
      END FUNCTION H2O_T2Cp

      FUNCTION H2O_H2T(h_in)
      ! Input:  Enthalpy (J/kg)
      ! Output: Temperature (K)
      use Inc_H2O, only: del_h, del_ho, h_t, max_h, min_h
#ifdef siarhei_fr
      use Mod_THfunc, only: ftemp
#endif

      implicit none
      REAL(8),INTENT(IN)                          :: h_in
      REAL(8)                                     :: h
      INTEGER(4)                                  :: ii
      REAL(8)                                     :: frac
      REAL(8)                                     :: H2O_H2T                              ! output

#ifdef siarhei_plot
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [H2O_H2T] in Mod_H2O'
...
#ifdef siarhei_fr
      H2O_H2T = ftemp(h_in) !@$^ siarhei_fr
#else
      h=max(min_h,min(max_h,h_in))
      ii=int((h-min_h)*del_ho+1)
      frac=(min_h+del_h*(ii)-h)*del_ho
      H2O_H2T=h_T(ii)*frac+h_T(ii+1)*(1.0d0-frac)
#endif
      return
      END FUNCTION H2O_H2T

      FUNCTION H2O_H2D(h_in)
      ! Input:  Enthalpy (J/kg)
      ! Output: Density (g/cc)
      use Inc_H2O, only: del_h, del_ho, h_den, max_h, min_h
#ifdef siarhei_fr
      use Mod_THfunc, only: fdensh
#endif
      implicit none
      REAL(8),INTENT(IN)                          :: h_in
      REAL(8)                                     :: h
      INTEGER(4)                                  :: ii
      REAL(8)                                     :: frac
      REAL(8)                                     :: H2O_H2D                              ! output

#ifdef siarhei_plot
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [H2O_H2D] in Mod_H2O'
...
#ifdef siarhei_fr
      H2O_H2D = fdensh(h_in) !@$^ siarhei_fr_test
#else
      h=max(min_h,min(max_h,h_in))
      ii=int((h-min_h)*del_ho+1)
      frac=(min_h+del_h*(ii)-h)*del_ho
      H2O_H2D=h_den(ii)*frac+h_den(ii+1)*(1.0d0-frac)
#endif
      return
      END FUNCTION H2O_H2D
      ! ----------------------------------------------------------------------------------------------

      ! CTF Water properties used in generating hash table
      ! ----------------------------------------------------------------------------------------------
      ...
      REAL(8)                                       :: v
      REAL(8)                                       :: boric_acid_ppm

#ifdef siarhei_plot
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [adjust_rhol_for_boron] in Mod_H2O'
      ...
      REAL(8)                                       :: gapt1
         ...
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [water_PT2Cp] in Mod_H2O'
      ...
      REAL(8)                                       :: pin6
         ...
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [water_PT2Cv] in Mod_H2O'
      ...
      REAL(8),PARAMETER                             :: H11=0.17887679267013D-06
      REAL(8),PARAMETER                             :: R1=461.526D-6
      ...
      REAL(8)                                       :: DVDPI
         ...
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [water_PT2DVDP] in Mod_H2O'
      ...
      function water_PDT2Eta(P,Di,T) result(Eta)
      ! T   is temperature in K
      ...
      REAL(8)                                       :: Dr
      REAL(8)                                       :: Dh
      REAL(8)                                       :: u0                                   ! u = u0*u1*u2 where u2=1 for ind. applications
      REAL(8)                                       :: u1                                   ! u = u0*u1*u2 where u2=1 for ind. applications
      INTEGER(4)                                    :: ii

#ifdef siarhei_plot
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [water_PDT2Eta] in Mod_H2O'
      ...
      REAL(8)                                       :: Tr                                   ! Normalized and Manipulated Temperature and Densities
      REAL(8)                                       :: Th                                   ! Normalized and Manipulated Temperature and Densities
      REAL(8)                                       :: D                                    ! Normalized and Manipulated Temperature and Densities
      REAL(8)                                       :: Dr                                   ! Normalized and Manipulated Temperature and Densities
      REAL(8)                                       :: Dh                                   ! Normalized and Manipulated Temperature and Densities
      REAL(8)                                       :: zeta1                                ! Calculated suport variables
      REAL(8)                                       :: zeta2                                ! Calculated suport variables
      REAL(8)                                       :: Chi                                  ! Calculated suport variables
      REAL(8)                                       :: xi                                   ! Calculated suport variables
      REAL(8)                                       :: y                                    ! Calculated suport variables
      REAL(8)                                       :: z                                    ! Calculated suport variables
      REAL(8)                                       :: mu                                   ! Calculated suport variables
      REAL(8)                                       :: cp                                   ! Calculated suport variables
      REAL(8)                                       :: cv                                   ! Calculated suport variables
      REAL(8)                                       :: cp_cv                                ! Calculated suport variables
      REAL(8)                                       :: k0                                   ! k=(k0*k1+k2) Base equation of calculation
      REAL(8)                                       :: k1                                   ! k=(k0*k1+k2) Base equation of calculation
      REAL(8)                                       :: k2                                   ! k=(k0*k1+k2) Base equation of calculation
      ! Index Counters
      INTEGER(4)                                    :: i
      INTEGER(4)                                    :: j
      INTEGER(4)                                    :: jj
         ...
         ! @$^ siarhei_fr
         current_sub = &
            '^#^ Entered [water_PDT2C] in Mod_H2O'
      ...
      mu=water_PDT2Eta(P,Di,T)      ! Dynamic Viscosity   [Pa-s]
